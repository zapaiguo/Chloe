...
namespace Chloe.Impls
{
    static class Helper
    {
        ...
        public static DbExpression StripInvalidConvert(DbExpression exp)
        {
            ...
            //(int?)123
            if (Helper.IsNullable(convertExpression.Type, out unType))//可空类型转换
            {
            ...
            if (exp.Type == typeof(int))
            {
                ...
                if (Helper.IsNullable(convertExpression.Operand.Type, out unType) && unType.IsEnum)
                    return StripInvalidConvert(convertExpression.Operand);
            }
            ...
            {
                //(long)NullableValue
                if (Helper.IsNullable(convertExpression.Operand.Type, out unType) && unType == exp.Type)
                    return StripInvalidConvert(convertExpression.Operand);
            }
