...
namespace ChloeTest
{
    ...
    public class FeatureTest
    {
        ...
        public static void CTest()
        {
            ...
            var reader = context.Session.ExecuteReader("SELECT AVG(Users.Id) AS C FROM Users AS Users");

            reader.Read();
            var t = reader.GetFieldType(0);


            ConsoleHelper.WriteLineAndReadKey(t.Name);
        ...
        public static void BulkCopyTest()
        {
            ...
            List<User> list = new List<User>() { new User {
                Id = 100005,
                Name = "test"
            } };
        ...
        static void MultKeysTest()
        {
            ...
            var x = TypeDescriptor.GetDescriptor(typeof(User));
            var xx = TypeDescriptor.GetDescriptor(typeof(MultKeyTable));

            result = context.QueryByKey<User>(1);
        ...
        static async void TaskTest()
        {
            var t = Task.FromResult<int>(TTTT());
        ...
        public static void SqlQueryTest()
        {
            ...
            context.SqlQuery<User>("select * from Users where Name=@name", new DbParam("@name", "lu")).ToList();
            context.SqlQuery<int>("select Id from Users where Name=@name", new DbParam("@name", "lu")).ToList();
        ...
        public static void JoinQueryTest()
        {
            ...
            ret = q.InnerJoin(context.Query<User>().Select(a => new { a.Id, a.Name }), (a, b) => q.First().Id > -1).Select((a, b) => new { A = a, B = b }).ToList();
            ...
            ret = q.InnerJoin(q1, (a, b) => a.Id == b.Id).Select((a, b) => new { a, b }).Where(a => a.a.Id > 0).ToList();
        ...
        public static void GroupQueryTest()
        {
            ...
            ret = q.Where(a => a.Id > 0).GroupBy(a => a.Id).AndBy(a => a.Age).Select(a => new { a.Id }).ToList();
        ...
        public static void UpdateTest()
        {
            ...
            SqlParameter p = new SqlParameter("@p", ts);
            p.DbType = DbType.Time;
            p.SqlDbType = SqlDbType.Time;
            r = context.Session.ExecuteNonQuery("update Users set F_Time=@p where id=0", new DbParam("@p", ts) { ExplicitParameter = p });

            //r = context.Update<User>(a => a.Id == 0, a => new User() { Name = a.Name, Age = a.Age + 100, Gender = Gender.Man, F_Time = ts, OpTime = DateTime.Now });
